...
export default function TicTacToe() {
  // odwolanie do elementu canvas w DOM
  ...
  const handleClick = (row: number, col: number) => {
    ...
    const newBoard = board.map((r, i) => r.map((c: any, j: number) => (i === row && j === col ? turn : c)));
    setBoard(newBoard);
    setTurn(turn === "X" ? "O" : "X");
  };

  // rysuje tablice w zaleznosci od stanu [board]
  useEffect(() => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen py-2 gap-16 w-screen">
      ...
      <canvas
        ...
        width={450}
        height={450}
        className="top-[512px] absolute pointer-events-none"
      />
    </div>
  );
}
